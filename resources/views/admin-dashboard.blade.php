                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <form action="/user/asignRole/{{ $user->id }}" method="post">
                                        @csrf
                                        @method('PUT')
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <select name="assignRole" id="">
    
                                                @foreach ($roles as $role)
                                                    <option @selected($user->hasRole($role->name))  value="{{ $role->id }}">{{ $role->name }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <button
                                                class="px-4 py-1.5 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Assign
                                                Role</button>
                                        </td>
                                    </form>
                                    <td class="px-6 py-4 whitespace-nowrap">
